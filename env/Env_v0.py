...
class CentralizedTwoRobotEnv(gym.Env):
    ...
    def __init__(self, max_episode_steps, group_id, robot_radius=0.5, render_mode=None):
        ...
        self._last_achieved_goal = self.state[:, :2].flatten().astype(np.float32).copy()
    ...
    def step(self, action):
        ...
        prev_achieved_goal = self._last_achieved_goal.copy()
        ...
        self._last_achieved_goal = obs["achieved_goal"].copy()
    ...
    def compute_reward(self, achieved_goal, desired_goal, info):
        """Return step penalty plus directional progress while preserving terminal bonuses."""
        achieved = self._reshape_goals(achieved_goal)
        desired = self._reshape_goals(desired_goal)
        batch_size = achieved.shape[0]

        rewards = np.full(batch_size, self.step_penalty, dtype=np.float32)

        prev_flat = self._extract_prev_achieved_goal(info, batch_size)
        if prev_flat is not None:
            previous = self._reshape_goals(prev_flat)
            prev_distances = np.linalg.norm(previous - desired, axis=2).sum(axis=1)
            current_distances = np.linalg.norm(achieved - desired, axis=2).sum(axis=1)
            rewards += self.directional_gain * (prev_distances - current_distances)
    ...
    def reset(self, *, seed=None, options=None):
        ...
        # Start each episode from the template configuration
        self.state = self._initial_state_template[self.robots].copy()
        self.goal_position = self._goal_template[self.robots].copy()
        self.step_count = 0
        self.episode_reward = 0.0
    ...
    def _extract_prev_achieved_goal(self, info, batch_size):
        key = "prev_achieved_goal"
        if info is None:
            return None
        if isinstance(info, dict):
            if key not in info:
                return None
            prev = np.asarray(info[key], dtype=np.float32)
            if prev.ndim == 1:
                prev = prev.reshape(1, -1)
            if prev.shape[0] == 1 and batch_size > 1:
                prev = np.repeat(prev, batch_size, axis=0)
            return prev
        collected = []
        for item in info:
