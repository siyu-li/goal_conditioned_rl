## Four robot individually controlled environment with centralized state/action space
...
class CentralizedThreeRobotEnv(gym.Env):
    metadata = {"render_modes": ["human", "rgb_array"], "render_fps": 30}

    def __init__(self, max_episode_steps, robot_radius=0.5, render_mode=None):
        super().__init__()
        ...
        # Initial state template for three robots
        ...
        # Goal template for three robots
        ...
        self.state = self._initial_state_template.copy()
        self.goal_position = self._goal_template.copy()
        self.step_count = 0
    ...
    def render(self):
        ...
        if self.render_mode == "human":
            plt.figure("CentralizedFourRobotEnv")
            plt.clf()
            plt.imshow(frame)
            plt.axis("off")
    ...
    def close(self):
        if self.render_mode == "human":
            plt.ioff()
            plt.close("CentralizedFourRobotEnv")
    ...
    def _get_obs(self):
        ...
        Observation includes relative positions to obstacles and other robots.
        """
        obs = []
        for obs_x, obs_y, _ in self.obstacles:
            for robot_idx in range(self.num_robots):
                robot_x, robot_y, robot_theta = self.state[robot_idx]
                obs.extend([robot_x - obs_x, robot_y - obs_y, robot_theta])
