...
## NOTE: Only boundary collisions are checked. Robots stop and receive collision penalty
## if they attempt to move outside workspace bounds. No obstacle or robot-robot collisions.
...
class CentralizedMultiRobotEnv(gym.Env):
    ...
    def __init__(self, max_episode_steps, num_robots, action_coupled=False, robot_radius=0.5, render_mode=None):
        super().__init__()
        ...
        # Observation space: robot states (x, y, theta) for each robot
        obs_dim = 3 * self.num_robots
    ...
    def step(self, action):
        ...
        if self.action_coupled:
            ...
            for i in range(self.num_robots):
                ...
                proposed_state[i, 0] = new_x
                proposed_state[i, 1] = new_y
                proposed_state[i, 2] = new_theta
        else:
            # Each robot has its own distance and heading change
            for i in range(self.num_robots):
                distance = float(action[2 * i])      # Distance for robot i
                dtheta = float(action[2 * i + 1])    # Heading change for robot i
                theta = self.state[i, 2]
                new_x = self.state[i, 0] + distance * np.cos(theta)
                new_y = self.state[i, 1] + distance * np.sin(theta)
                new_theta = self._wrap_angle(theta + dtheta)
                proposed_state[i, 0] = new_x
                proposed_state[i, 1] = new_y
                proposed_state[i, 2] = new_theta

        # Check for boundary violations (collision with workspace boundaries)
        collision = self._check_boundary_collision(proposed_state)
        
        # Only update state if no boundary collision
        if not collision:
    ...
    def compute_reward(self, achieved_goal, desired_goal, info):
        ...
        # Apply collision penalty for boundary violations
    ...
    def compute_terminated(self, achieved_goal, desired_goal, info):
        """Terminate when all robots reach goals or a boundary collision occurs."""
        success_mask = self._success_mask(achieved_goal, desired_goal)
        batch_size = success_mask.shape[0]
        # Terminate on boundary collision
        collision_mask = self._extract_flag_from_info(info, "collision", False, batch_size)
    ...
    def reset(self, *, seed=None, options=None):
        ...
        # Randomly sample initial state and goal positions
        self.state = self._sample_random_state()
        self.goal_position = self._sample_random_goals()
        self.step_count = 0
        self.episode_reward = 0.0
    ...
    def _check_boundary_collision(self, state):
        """Check if any robot violates workspace boundaries."""
        for i in range(self.num_robots):
            x, y = state[i, :2]
    ...
    def _sample_random_state(self):
        """
        Sample random initial state for all robots.
    ...
    def _sample_random_goals(self):
        """
        Sample random goal positions for all robots.
        ...
        for i in range(self.num_robots):
            goals[i] = self._sample_random_position()
    ...
    def _get_obs(self):
        """
        Constructs observation dictionary with robot states.
        Observation includes (x, y, theta) for each robot.
        """
        # Flatten robot states: [x1, y1, theta1, x2, y2, theta2, ...]
