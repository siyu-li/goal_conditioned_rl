...
class CentralizedMultiRobotEnv(gym.Env):
    ...
    def __init__(self, max_episode_steps, num_robots=3, robot_radius=0.5, render_mode=None):
        super().__init__()
        ...
        # Observation space: robots and goals expressed relative to obstacles
        ...
        self.proximity_threshold = 2.5  # Distance threshold for proximity penalty
        self.max_proximity_penalty = -10.0  # Maximum penalty when robots are at collision distance

        # Initialize with random state and goals
    ...
    def _build_observation_bounds(self):
        ...
        - For each obstacle: robot_obs_rel_x, robot_obs_rel_y
        - cos(robot_theta), sin(robot_theta)
        - nearest_robot_distance
        - cos(nearest_robot_polar_angle), sin(nearest_robot_polar_angle)
        
        Plus goals relative to obstacles (same as before).
        """
        obs_low = []
        obs_high = []
        ...
        for _ in range(self.num_robots):
            # Relative position to each obstacle (2 values per obstacle)
            ...
            # Nearest neighbor: distance, cos(angle), sin(angle)
            obs_low.extend([0.0, -1.0, -1.0])
            obs_high.extend([np.sqrt(2) * max(self.x_max, self.y_max), 1.0, 1.0])
        
        # Goals relative to each obstacle (same as before)
        for _ in range(self.num_obstacles):
            for _ in range(self.num_robots):
                obs_low.extend([-self.x_max, -self.y_max])
                obs_high.extend([self.x_max, self.y_max])
    ...
    def compute_reward(self, achieved_goal, desired_goal, info):
        ...
        # Add proximity penalty for robots that are too close
    ...
    def _compute_nearest_neighbor_info(self):
        """
        For each robot, compute the distance and polar angle to its nearest neighbor.
        
        Returns:
            nearest_info: (num_robots, 3) array with [distance, cos(polar_angle), sin(polar_angle)]
                         for each robot's nearest neighbor
        """
        nearest_info = np.zeros((self.num_robots, 3), dtype=np.float32)
        ...
        for i in range(self.num_robots):
            ...
            # Find the nearest neighbor among all other robots
            for j in range(self.num_robots):
                ...
                if distance < min_distance:
                    ...
                    # Compute polar angle of the relative vector
                    nearest_angle = np.arctan2(rel_vec[1], rel_vec[0])
            
            # Store distance and polar angle (as cos and sin)
            nearest_info[i, 0] = min_distance if min_distance != float('inf') else 0.0
            nearest_info[i, 1] = np.cos(nearest_angle)
            nearest_info[i, 2] = np.sin(nearest_angle)
    ...
    def _get_obs(self):
        ...
        - robot_obs_rel_x, robot_obs_rel_y (for each obstacle)
        - cos(robot_theta), sin(robot_theta)
        - nearest_robot_distance
        - cos(nearest_robot_polar_angle), sin(nearest_robot_polar_angle)
        
        Plus goals relative to obstacles at the end.
        """
        obs = []
        
        # Compute nearest neighbor info for all robots
        nearest_info = self._compute_nearest_neighbor_info()
        
        # Per robot observations
        for robot_idx in range(self.num_robots):
            robot_x, robot_y, robot_theta = self.state[robot_idx]
            
            # Relative position to each obstacle
            for obs_x, obs_y, _ in self.obstacles:
                obs.extend([robot_x - obs_x, robot_y - obs_y])
            ...
            obs.extend([
                nearest_info[robot_idx, 0],  # distance
                nearest_info[robot_idx, 1],  # cos(polar_angle)
                nearest_info[robot_idx, 2]   # sin(polar_angle)
            ])
        
        # Goals relative to each obstacle
        for obs_x, obs_y, _ in self.obstacles:
            for goal_idx in range(self.num_robots):
                goal_x, goal_y = self.goal_position[goal_idx]
                obs.extend([goal_x - obs_x, goal_y - obs_y])
