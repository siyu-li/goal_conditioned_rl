...
class CentralizedMultiRobotEnv(gym.Env):
    ...
    def __init__(self, max_episode_steps, num_robots=3, action_coupled=False, robot_radius=0.5, render_mode=None):
        super().__init__()
    ...
    def is_valid_state(self, state):
        """Check boundary, and inter-robot collision constraints."""
        for i in range(self.num_robots):
            x, y = state[i, :2]
    ...
    def is_valid_motion(self, proposed_state):
        """Reject straight-line motions that would intersect or other robots."""
        for i in range(self.num_robots):
    ...
    def _is_position_valid(self, pos, existing_positions):
        """
        Check if a position is valid (no collision with existing positions).
